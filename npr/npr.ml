...
let base_uri station_id =
  sprintf "https://api.composer.nprstations.org/v1/widget/%s/playlist" station_id
  |> Uri.of_string
;;
...
module Playlist = struct
  type playlist = { playlist : Song.Internal.t list }
...
let create ~debug_mode ~station_id =
  let cohttp_request_async_client =
    Cohttp_request_async_client.create
      ~debug_mode:
        (if debug_mode
        then Some { print_requests = true; print_responses = false }
        else None)
  in
...
let query t time =
  let date, time_ofday = Time_ns.to_date_ofday time ~zone:(force Time_ns.Zone.local) in
...
let add_some_time time =
  Time_ns.min (Time_ns.now ()) (Time_ns.add time (Time_ns.Span.of_min 30.))
;;
...
let lookup_until_done t writer ~from ~done_ ~in_range =
  let rec loop time last_time song_ids backoff =
    ...
    then Deferred.unit
      ...
      let%bind songs =
        Cohttp_request_async_client.request t.cohttp_request_async_client request
      in
      match songs with
      ...
      | Ok songs ->
        ...
        | Ok new_songs ->
          ...
          let new_time =
            let new_time =
              if Time_ns.compare new_time last_time < 0 then last_time else new_time
            in
