...
module Get_access_token = struct
  let get_access_token
    ...
    | Error (`Msg error) ->
      failwithf
        "Base64 encoding of the credentials failed -- bug in Base64 library? Original \
         error: %s"
        error
        ()
    | Ok base64_encoded ->
...
module Authorization_code_flow = struct
  module Authorization_code = struct
    ...
    let of_code code ~redirect_uri = { redirect_uri; code }
  end
  ...
  let generate_client_uri_for_authorization_code ~client_id ~redirect_uri ~scopes =
  ...
  let refresh_access_token refresh_token ~credentials =
    let module Access_token_without_refresh_token = struct
      type t =
        { access_token : Access_token.t
        ; expires_in : int
        }
    ...
    |> Cohttp_request.map
         ~f:(fun { Access_token_without_refresh_token.access_token; expires_in } ->
...
module Tracks = struct
  type t = { tracks : Track.t Paging_object.t } [@@deriving yojson { strict = false }]
end

module Playlist = struct
  type t = { id : string } [@@deriving yojson { strict = false }]
...
let request ~access_token ~uri ~request ~response =
  { Cohttp_request.uri
  ; headers = Cohttp.Header.of_list [ "Authorization", sprintf "Bearer %s" access_token ]
...
let make_playlist ~kind ~access_token ~user_id ~name =
  ...
  in
  let module Playlist_body = struct
    type t =
      { name : string
      ; public : bool
      }
...
let add_to_playlist ~access_token ~(playlist : Playlist.t) ~(tracks : Track.t list) =
  ...
  in
  let module Add_to_playlist_body = struct
    type t = { uris : string list } [@@deriving yojson]
  end
  in
  let body = { Add_to_playlist_body.uris = List.map tracks ~f:(fun t -> t.uri) } in
  request
    ~access_token
    ~uri
    ~request:
      (Post (Json_encoded { body; body_to_yojson = (module Add_to_playlist_body) }))
...
let search_tracks ?offset ?limit () ~access_token ~query =
  let uri =
    ...
    Uri.add_query_params
      base_uri
      ([ "q", [ query ]; "type", [ "track" ] ] @ paging_query_params ~offset ~limit)
  in
  request ~access_token ~uri ~request:Get ~response:(module Tracks)
...
let lookup_playlist ?offset ?limit () ~access_token ~(playlist : Playlist.t) =
  let uri =
    sprintf "https://api.spotify.com/v1/playlists/%s/tracks" playlist.id |> Uri.of_string
  in
